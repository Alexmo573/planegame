...
import random
import cv2
from mediapipe_gestuer_helper import mediapipe_gestuer_helper
import pygame.font
import os
...
class Player(pygame.sprite.Sprite):
    ...
    def move(self):
        self.rect.left = mid(0, self.rect.left + dx*self.speed*30 , SCREEN_WIDTH - self.rect.width)
        self.rect.top = mid(0,self.rect.top + dy*self.speed*30 , SCREEN_HEIGHT - self.rect.height)
...
gestuer_helper = mediapipe_gestuer_helper()
# 初始化 pygame
pygame.init()
        ...
        if score < 25:
            ...
            if not player.is_hit:
                ...
                if shoot_frequency % 5 == 0:
                    gestuer_helper.take_photo()
                    detect_result = gestuer_helper.get_last_hand_info()
                    if detect_result is not None:
                        gesture,x,y,z = detect_result
                        ...
                        move = False
                    cv2.imshow('frame', gestuer_helper.last_image)  # 显示摄像头画面
